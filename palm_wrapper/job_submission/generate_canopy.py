...
def get_xarray_ds(job_name, tree_matrix, dx, dy, dz, mean_lai, stack_height, topo, **kwargs):
    ...
    ds["zt"] = get_topo_dataarray(topo, ds.y, ds.x)
    # ssws = setup_surface_scalar_np(topo, stack_height)
    # ds["ssws"] = add_surface_scalar_dataarray(ds, ssws, ds.y, ds.x)
    return ds


def get_topo_dataarray(topo, y, x):
...
def get_surface_scalar_dataarray(ds, ssws, y, x):
