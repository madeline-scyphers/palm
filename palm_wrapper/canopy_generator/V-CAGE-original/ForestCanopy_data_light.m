function [CSProfile, HDBHpar, LADcm,zcm, avg, avgH, sig, sigH, StandDencity, AcF]=...
    ForestCanopy_data_l(ptype, nx, Dx, ny, Dy)
...
if ptype==1 % 1=Duke forest loblolly pine December 2001, 
    ...
    sig = 0.227; %LAI standard deviation
...
elseif ptype==2 %arbitrary grass patch
    ...
    sig= 0.01; %LAI standard deviation
...
elseif ptype==3 %hardwood duke forest march-april
    ...
    sigH=  3.79; %std of heights
    
    avg= 2.950; %the mean LAI 
    sig= 0.257*(sigH/3.79); %LAI standard deviation
    
    Bowen = 1.25; %apr 18 2002
    sigBowen = 0.16*(sigH/3.79); %apr 18 2002
    LAI = LAI/sum(LAI);
    lencm = double(ceil(max(zlai)*100))/double(canopytopCM);
    LADcm = interp1((zlai/lencm),LAI,zcm); 
    LADcm=LADcm/sum(LADcm);
