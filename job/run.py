...
def create_input_files(config):
    ...
    ds = get_lad_netcdf(job_name=config["job_name"], tree_matrix=domain.trees_matrix, dx=config["domain"]["dx"], dy=config["domain"]["dy"], dz=config["domain"]["dz"], mean_lai=config["canopy"]["mean_lai"])
