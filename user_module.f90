...
!------------------------------------------------------------------------------!
 MODULE user
    ...
    !REAL(wp) ::  Canopy1_edge_right  =  0.0_wp    !< right edge of canopy in meters

    !REAL(wp) ::  Canopy2_edge_north  =  0.0_wp    !< left edge of canopy in meters
    !REAL(wp) ::  Canopy2_edge_south  =  0.0_wp    !< right edge of canopy in meters
    ...
    SELECT CASE ( location )
       ...
       CASE ( 'before_timestep' )
            ...
            DO  m = 1, surf_def_v(0)%ns
               ...
               ! surf_def_v(0)%ssws(m) = 1.1
               surf_def_v(1)%ssws(m) = 100.1
               ! surf_def_v(2)%ssws(m) = 1.1
               ! surf_def_v(3)%ssws(m) = 1.1
